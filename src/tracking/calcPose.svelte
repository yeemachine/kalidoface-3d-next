<script context="module">
  ...
  import {user_smooth_pose} from './smooth.svelte';
  import {cameraConfig} from '../scene/CameraControls.svelte'


export let calcPose = (lm,lmv) => {
  const oldPose = get(user_smooth_pose)
...
//       lowerLeg.l.x = 0
      ...
      let leftHandOffscreen = lm[15].y > -.1 || lm[15].visibility < .23 || .995 < lmv[15].y 
      let rightHandOffscreen = lm[16].y > -.1 || lm[16].visibility < .23 || .995 < lmv[16].y 
