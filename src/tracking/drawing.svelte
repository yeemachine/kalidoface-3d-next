...
export function drawResults(results,type="holistic",facelm=null) {
  ...
  switch (type){
    ...
    case 'pose':
        if(facelm){
           let lm = []
           for(let i=0;i<facelm.scaledMesh.length;i++){
             lm.push({
               visibility:facelm.faceInViewConfidence,
               x:facelm.scaledMesh[i][0]/videoEl.videoWidth,
               y:facelm.scaledMesh[i][1]/videoEl.videoHeight,
               z:facelm.scaledMesh[i][2]/videoEl.videoWidth
              })
           }
        ...
        drawConnectors(canvasCtx, results.keypoints, POSE_CONNECTIONS, {
          color: "#00cff7",
          lineWidth: 4
        });
        drawLandmarks(canvasCtx, results.keypoints, {
          color: "#ff0364",
          lineWidth: 2
        });
      break;
