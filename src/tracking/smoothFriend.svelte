...
export let friend_smooth_face = spring({
  eye:{l:1,r:1},
...
export let friend_smooth_pose = spring({
...
friend_smooth_pose.subscribe(val=>{
  if(val){
    const pose = get(profiles).friend.tracking.pose
...
export let friend_smooth_hand = {Right:{},Left:{}}

for (let i=0; i<2; i++){
  let e = i==0 ? 'Right' : 'Left'
  friend_smooth_hand[e] = spring({
    Wrist:{x:0,y:0,z:0},
  ...
  friend_smooth_hand[e].subscribe(val=>{
    if(val && profiles){
      const hand = get(profiles).friend.tracking.hand
