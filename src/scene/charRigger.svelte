...
export const animateHead = (currentVrm,face) => {
  ...
  const Chest = currentVrm.humanoid.getBoneNode( VRMSchema.HumanoidBoneName.Spine )
  //offset from pivot
  ...
  let offsetY = get(options).holistic.enable && Chest ? Chest.rotation.y*2 : 0
  
  //turn LR
  if(Neck){
    Neck.rotation.y = clamp((face.head.x/30),-.8,.8) - offsetY ; 
    //lean forward/back
...
export const animateChest = (currentVrm,face,pose) => {
  ...
  upperChest.rotation.z = clamp((face.head.z/90*.5),-.3,.3);
    upperChest.rotation.y = get(options).holistic.enable ? clamp((pose.Spine.x/50*dampener),-.3,.3) 
    : clamp((face.head.x/50*.2*dampener),-.3,.3);
  ...
  Chest.rotation.y = get(options).holistic.enable ? clamp((pose.Spine.x/20*dampener),-.7,.7)
    : clamp((face.head.x/50*dampener),-.3,.3);
  }

  // console.log(pose.Spine.x)
  if(Spine){
  Spine.rotation.x = clamp((face.head.y/50*dampener),-.3,.3);
  Spine.rotation.z = clamp((face.head.z/90*dampener),-.3,.3);
  Spine.rotation.y = get(options).holistic.enable  ? clamp((pose.Spine.x/20*dampener),-.4,.4)
    : clamp((face.head.x/50*dampener),-.3,.3);
  }
...
export const animateArm = (currentVrm,pose,side="Right",flipHorizontal = false) => {
    ...
    UpperArm.rotation.y = pose[side+'UpperArm'].y * 3.14 / 180* invert;
    if(flipHorizontal){
      UpperArm.rotation.y += (pose[side+'LowerArm'].x * 1/180)
      UpperArm.rotation.y += (-invert*Math.max(pose[side+'LowerArm'].z,0) * 1/180) 
    }else{
      UpperArm.rotation.y -= Math.max(pose[side+'LowerArm'].x * .4/180)
      UpperArm.rotation.y -= (-invert*Math.max(pose[side+'LowerArm'].z,0) * .2/180) 
    }
   
  
    UpperArm.rotation.x = pose[side+'UpperArm'].x * .3 / 180 * invert;
    ...
    LowerArm.rotation.y = pose[side+'LowerArm'].y * 3.14/180 * invert ; 
    LowerArm.rotation.x =pose[side+'LowerArm'].x * 2.14/180* invert * flipAgain; //prob need to adjust X
    LowerArm.rotation.x = clamp(LowerArm.rotation.x,-.3,.3)
...
export const animateHips = (currentVrm,face) => {
  ...
  Hips.position.x = currentVrm.humanoid.restPose.hips.position[0] - clamp((Spine.rotation.z/90),-1,1)*-posXMult;  
  ...
  Hips.rotation.y = Math.PI + clamp(Chest.rotation.y,-1.3,1.3)*.75
}
