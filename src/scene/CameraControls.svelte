...
import { animateArm, animateHead, animateFace } from './charRigger.svelte';
...
export const faceToCam = (obj, cam, type = 'user') => {
  ...
  if(!freeCamera){
    ...
    animateHead(obj, get(profiles)[type].tracking.Face,get(profiles)[type].tracking.Pose);
    return
  }
...
export const panZoomCam = (camera) => {
  if (!get(cameraConfig).firstPerson) {
        ...
        let offsetX = clamp((get(profiles).user.tracking.Face.head.position.x - 0.5) * 0.35, -1.5, 1.5);
        let offsetY = clamp((get(profiles).user.tracking.Face.head.position.y - 0.5) * 0.35, -1.5, 1.5);
