...
import {stickerLayers,stickerList,presetStickers} from '../scene/stickers.svelte'
...
<container
  ...
  <div class="sticker-list"
     ...
     out:fade="{{duration:$DIM.w > 600 ? 200 : 0}}"
       ...
       <div class="upload">
      <label data-text="{subnav_sticker_text.upload[$setLang]}" for="uploadImage">
      ...
      <label data-text="Load from URL" on:click={getStickerFromPrompt}>
        <i class="svgIcon"><SvgLink/></i>
      </label>
    </div>
    

    {#each [...$stickerList,...presetStickers] as sticker,i (sticker)}
